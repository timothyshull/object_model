int _main() {
    ...
    D::D(&var_20);
    memset(&var_40, 0x0, 0x8);
    B::B();
    rax = __dynamic_cast(&var_40, typeinfo for B, typeinfo for D, 0x0);
    var_88 = rax;
    if (rax == 0x0) {
            __cxa_bad_cast();
    }
...
function sub_100000f70() {
    *(rbp + 0xffffffffffffff90) = rax;
    *(rbp + 0xffffffffffffff88) = std::__1::basic_ostream<char, std::__1::char_traits<char> >& std::__1::operator<< <std::__1::char_traits<char> >(*(rbp + 0xffffffffffffff90), 0xa);
    __cxa_end_catch();
    return;
}

function sub_100001680() {
